...
import { subscribeToUsers, updateUserData, registerUser } from '../services/firebaseService'; // Canlı servisler
...
export const AdminPanel: React.FC<AdminPanelProps> = ({ onBack }) => {
  ...
  useEffect(() => {
    const unsubscribe = subscribeToUsers((liveUsers) => {
      ...
      // Eğer bekleyen kullanıcı varsa otomatik oraya geç
      if (liveUsers.some(u => !u.isApproved)) {
        setActiveTab((prev) => prev === 'users' ? 'pending' : prev);
      }
    });
    return () => unsubscribe();
  }, []);
  ...
  const handleSaveCreate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Not: registerUser otomatik olarak Firestore'a ekler
      // Ancak rol ve onay durumunu güncellememiz gerekebilir
      // Şimdilik basit tutuyoruz, registerUser içindeki default değerler oluşacak
      setIsCreatingUser(false);
      alert("Kullanıcı oluşturuldu! Not: Şifre Firebase Auth'da saklanır.");
    } catch (error: any) {
  ...
  const handleApprove = async (user: User) => {
    await updateUserData(user.id, { isApproved: true });
  };
  ...
  // Helper for rendering permissions (Only needed for Edit now)
        ...
        {BRANDS.map(brand => {
          ...
          return (
            <button
              key={brand.id}
              type="button"
              onClick={() => toggleBrandPermission(brand.name, true)}
